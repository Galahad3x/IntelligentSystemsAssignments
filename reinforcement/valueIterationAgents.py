...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for _ in range(self.iterations):
            # make a copy of the values to mantain the values of k - 1iteration
    ...
    def computeActionFromValues(self, state):
        ...
        """
        # mark as ilegal the states without legal actions
        if self.mdp.isTerminal(state):
            return None
        actions = self.mdp.getPossibleActions(state)
        # compute all q_values
        q_values = {action: self.computeQValueFromValues(state, action)  
                     for action in actions}
        # get action with max q_value
        return util.Counter(q_values).argMax()
